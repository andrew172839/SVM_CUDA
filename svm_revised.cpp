...
int Solver::select_working_set(int &out_i, int &out_j)
{
	...
	if (i != -1) //q_i not accessed: gmax = -inf if i = -1
	{
...
int Solver_NU::select_working_set(int &out_i, int &out_j)
{
	...
	if (ip != -1) //q_ip not accessed: gmaxp = -inf if ip = -1
	{
...
static void solve_one_class(const svm_problem *prob, const svm_parameter *param, double *alpha, Solver::SolutionInfo *si)
{
	...
	int i;
	int n = (int)(param->nu * prob->l); //# alpha's at upper bound
	for (i = 0; i < n; i++)
	{
		alpha[i] = 1;
	}
...
static double svm_svr_probability(const svm_problem *prob, const svm_parameter *param)
{
	...
	mae /= (prob->l - count);
	info("prob. model for test data: target value = predicted value + z,\nz: laplace distribution e ^ (-|z| / sigma) / (2 * sigma), sigma= %g\n", mae);
	free(ymv);
	return mae;
}
...
static void svm_group_classes(const svm_problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	...
	//however, for two-class sets with -1 / +1 labels and -1 appears first, we swap labels to ensure that internally the binary svm has positive data corresponding to the +1 instances
